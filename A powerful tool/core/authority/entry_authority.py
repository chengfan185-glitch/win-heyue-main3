...
class EntryAuthority:
    ...
    def __init__(self):
        ...
        # Runtime snapshot (for global hold / diagnostics)
        self._last_payload_time_utc: Optional[datetime] = None
        self._last_global_hold: bool = False
    ...
    def read_intel(self) -> Tuple[Optional[datetime], List[IntelSignal]]:
        ...
        # Persist snapshot
        self._last_payload_time_utc = intel_time
        self._last_global_hold = bool(payload.get("global_hold") is True)
        self._last_hold_reason = str(payload.get("hold_reason") or payload.get("reason") or "").strip()
        ...
        if isinstance(items, dict):
            ...
            for it in items:
                if isinstance(it, str):
                    ...
                    if not sym:
                        continue
                    # Support newer intel formats that expose allowed_actions instead of a direct direction.
                    direction = str(it.get("direction") or it.get("side") or payload.get("direction") or "").upper().strip()
                    if not direction or direction == "HOLD":
                        aa = it.get("allowed_actions") or it.get("allowed") or []
                        if isinstance(aa, str):
                            aa = [aa]
                        aa_norm = [str(x).upper() for x in aa if str(x).strip()]
                        if "LONG" in aa_norm and "SHORT" in aa_norm:
                            # ambiguous -> HOLD (trend system should not issue both)
                            direction = "HOLD"
                        elif "LONG" in aa_norm:
                            direction = "LONG"
                        elif "SHORT" in aa_norm:
                            direction = "SHORT"
                        else:
                            direction = "HOLD"

                    # Confidence: accept confidence/conf; otherwise fall back to score (normalized to [0,1]).
                    conf_raw = it.get("confidence", it.get("conf", None))
                    if conf_raw is None:
                        conf_raw = it.get("score", payload.get("confidence", 0.0))
                    try:
                    ...
                    if conf < 0:
                        conf = 0.0
                    if conf > 1:
                        # If score is not already 0..1, squash gently.
                        conf = conf / (abs(conf) + 1.0)
    ...
    def is_global_hold(self) -> Tuple[bool, str]:
        """Return (global_hold, reason)."""
        return bool(self._last_global_hold), str(self._last_hold_reason or "").strip()
    ...
    def get_entry_instruction(self, symbol: str, signals: List[IntelSignal], now_utc: datetime) -> Tuple[str, float, str]:
        ...
        gh, gh_reason = self.is_global_hold()
        if gh:
            return "HOLD", 0.0, f"global_hold:{gh_reason}" if gh_reason else "global_hold"
