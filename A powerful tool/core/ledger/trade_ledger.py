...
class TradeLedger:
    ...
    def __init__(self, base_dir: str = "logs/ledger"):
        ...
        # Hydrate open positions from disk (critical for reconciliation)
        self._sync_open_positions_from_disk()
    ...
    def _sync_open_positions_from_disk(self) -> None:
        """
        Load positions.jsonl and reconstruct current OPEN positions into memory.

        This prevents ghost mismatches where the exchange has positions but the local ledger
        starts with an empty in-memory map (which would trigger CLOSE_ONLY mode forever).
        """
        try:
            latest = self._latest_positions_map()
            self._open_positions = {sym: pos for sym, pos in latest.items() if getattr(pos, 'status', None) == 'OPEN'}
        except FileNotFoundError:
            self._open_positions = {}
        except Exception:
            logger.exception('[LEDGER] failed to hydrate open positions from disk')
            self._open_positions = {}

    # ---- Compatibility helpers (for scripts / one-liners) ----
    @property
    def positions(self):
        """Alias to open positions map (symbol -> Position)."""
        return self._open_positions

    @property
    def positions_path(self) -> str:
        return str(self.positions_file)

    def adopt_exchange_positions_raw(self, raw_positions, source: str = 'exchange_adopt', overwrite: bool = False) -> int:
        """
        Adopt current exchange positions into the local ledger as OPEN positions.

        - raw_positions: list of dicts from Binance positionRisk endpoint (symbol, positionAmt, entryPrice, leverage...)
        - overwrite: if True, will overwrite existing local OPEN position for a symbol.

        Returns number of adopted symbols.
        """
        adopted = 0
        now = datetime.now(timezone.utc).timestamp()
        for rp in raw_positions or []:
            try:
                amt = float(rp.get('positionAmt') or 0)
            except Exception:
                continue
            if amt == 0:
                continue
            sym = str(rp.get('symbol') or '').strip()
            if not sym:
                continue
            if (not overwrite) and (sym in self._open_positions):
                continue
            side = 'LONG' if amt > 0 else 'SHORT'
            try:
                entry_price = float(rp.get('entryPrice') or 0)
            except Exception:
                entry_price = 0.0
            try:
                leverage = int(float(rp.get('leverage') or 1))
            except Exception:
                leverage = 1
            pos = Position(
                position_id=f'adopt_{sym}_{uuid.uuid4().hex[:10]}',
                symbol=sym,
                side=side,
                ...
                leverage=leverage,
                run_id=self.run_id,
                entry_features={'source': source}
            )
            # persist + set in-memory
            self._append_jsonl(self.positions_file, pos)
            self._open_positions[sym] = pos
            adopted += 1
        return adopted

    def adopt_exchange_position_info(self, pinfo, source: str = 'exchange_adopt', overwrite: bool = False) -> bool:
        """Adopt a PositionInfo (from adapter.get_position) into local OPEN positions."""
        sym = getattr(pinfo, 'symbol', None)
        if not sym:
            return False
        if (not overwrite) and (sym in self._open_positions):
            return False
        qty = float(getattr(pinfo, 'quantity', 0) or 0)
        if qty == 0:
            return False
        side = str(getattr(pinfo, 'side', 'LONG'))
        entry_price = float(getattr(pinfo, 'entry_price', 0) or 0)
        lev = int(getattr(pinfo, 'leverage', 1) or 1)
        now = datetime.now(timezone.utc).timestamp()
        pos = Position(
            position_id=f'adopt_{sym}_{uuid.uuid4().hex[:10]}',
            symbol=sym,
    ...
    def mark_position_stale(self, symbol: str, note: str = 'AUTO_HEAL') -> bool:
        """Mark a local OPEN position as CLOSED without executing on exchange.

        This is used for recovery when the local ledger contains ghost OPEN records that do not exist on the exchange.
        Because positions.jsonl is append-only, we append a CLOSED record that supersedes the previous OPEN record.
        """
        latest = self._latest_positions_map()
        pos = latest.get(symbol)
        if not pos or pos.status != 'OPEN':
            return False
        try:
            pos.status = 'CLOSED'
            pos.closed_at = time.time()
            pos.close_order_id = f'{note}'
            # keep prices as-is; realized pnl is unknown => set 0.0
            pos.realized_pnl = 0.0
            self._append_jsonl(self.positions_file, pos)
            return True
        except Exception as e:
            print(f'[LEDGER ERROR] mark_position_stale failed for {symbol}: {e}')
            return False
    ...
    def load_all_positions(self) -> List[Position]:
        ...
        return positions
    def _latest_positions_map(self) -> Dict[str, Position]:
        """Return latest Position record per symbol (append-only safe).

        The positions.jsonl file is append-only; a later CLOSED record should supersede an earlier OPEN record.
        """
        latest: Dict[str, Position] = {}
        for pos in self.load_all_positions():
            # overwrite, so the last record wins
            latest[pos.symbol] = pos
        return latest

    def reconcile_positions(self, exchange_positions: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Reconcile local positions with exchange (append-only safe)
        """
        ...
        latest = self._latest_positions_map()
        local_positions = {sym: p for sym, p in latest.items() if p.status == 'OPEN'}
