    ...
    try:
        while True:
            ...
            if entry_authority.use_intel and intel_signals:
                # Some EntryAuthority versions expect different intel_signals shapes.
                # We treat our canonical `intel_signals` as: {SYMBOL: {action/confidence/reason...}}
                try:
                    intel_universe = entry_authority.resolve_symbol_universe(env_symbols, intel_signals)
                except Exception as e:
                    # Fallback: use intel keys directly (prevents crashes like: 'str' object has no attribute 'symbol')
                    intel_universe = list(intel_signals.keys())
                    print(f"[INTEL] resolve_symbol_universe failed: {type(e).__name__}: {e} -> fallback to intel_signals keys ({len(intel_universe)})")
            elif entry_authority.use_intel and entry_authority.intel_symbols_strict:
                intel_universe = []
            else:
                intel_universe = env_symbolsols
